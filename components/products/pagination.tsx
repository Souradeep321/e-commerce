...
interface PaginationProps {
  ...
  totalPages: number;
  searchParams: Record<string, string | undefined>;
  // Defaults to "/products" so existing callers there are unaffected.
  // Other consumers (e.g. /categories/[slug]) pass their own route so
  // page links stay scoped correctly.
  basePath?: string;
}

export function Pagination({ currentPage, totalPages, searchParams, basePath = "/products" }: PaginationProps) {
  if (totalPages <= 1) return null;
  ...
  return (
    <nav aria-label="Pagination" className="mt-12 flex items-center justify-center gap-2">
      <Button variant="outline" size="icon" disabled={currentPage <= 1} asChild={currentPage > 1}>
        {currentPage > 1 ? (
          <Link href={buildHref(searchParams, currentPage - 1, basePath)} aria-label="Previous page">
            <ChevronLeft className="h-4 w-4" />
          </Link>
        ) : (
          <ChevronLeft className="h-4 w-4" />
        ...
        page === "ellipsis" ? (
          ...
          >
            {page === currentPage ? (
              ...
              <Link href={buildHref(searchParams, page, basePath)}>{page}</Link>
            )}
          </Button>
        )
      )}
      ...
      >
        {currentPage < totalPages ? (
          <Link href={buildHref(searchParams, currentPage + 1, basePath)} aria-label="Next page">
            <ChevronRight className="h-4 w-4" />
          </Link>
        ) : (
          <ChevronRight className="h-4 w-4" />
...
function buildHref(searchParams: Record<string, string | undefined>, page: number, basePath: string): string {
  const params = new URLSearchParams();
  for (const [key, value] of Object.entries(searchParams)) {
    if (value !== undefined && key !== "page") params.set(key, value);
  }
