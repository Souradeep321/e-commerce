...
import { ChevronRight, Heart, User, Package, HelpCircle } from 'lucide-react'
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export function MobileMenu({ open, onClose }: MobileMenuProps) {
  return (
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent
        side="left"
        className="w-full max-w-sm overflow-y-auto p-0 sm:w-80"
      >
        <SheetHeader className="border-b border-gray-200 px-5 py-4">
          <SheetTitle className="font-serif text-xl tracking-tight text-gray-900">
            AURVA
          </SheetTitle>
        </SheetHeader>
        ...
        <div className="flex flex-col">
          {menuSections.map((section) => (
            <div key={section.title} className="border-b border-gray-200 py-2">
              <p className="px-5 py-2 text-xs font-semibold uppercase tracking-wider text-gray-500">
                {section.title}
              </p>
              ...
              {section.links.map((link) => (
                <Link
                  ...
                  href={link.href}
                  onClick={onClose}
                  className="flex items-center justify-between px-5 py-3 text-sm font-medium text-gray-900 transition-colors hover:bg-gray-100"
                >
                  {link.label}
